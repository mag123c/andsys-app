...
- [x] 캐릭터 패널 (우측)
  - [x] CharacterPanel 컴포넌트
  - [x] 드래그 가능한 캐릭터 항목
  - [x] 이미 그래프에 있는 캐릭터 표시
- [x] 노드 드래그 생성
  - [x] onDrop, onDragOver 이벤트 처리
  - [x] 드롭 위치에 CharacterNode 생성
- [x] 간선 드래그 연결
  - [x] 노드 핸들에서 드래그
  - [x] onConnect 시 RelationshipDialog 표시
  - [x] 기존 관계 있으면 편집 모드
- [ ] 노드 삭제 (보류 - 실수 방지 위해 제거)
  - [ ] 노드 선택 시 삭제 버튼
  - [ ] 드래그 아웃으로 삭제
  - [ ] 연결된 관계도 함께 삭제
- [x] 양방향 엣지 개선
