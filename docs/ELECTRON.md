...
웹과 달리 Electron 앱은 **로컬 저장 기본, 수동 동기화** 방식 채택.

```
저장 → IndexedDB (즉시, 항상 로컬)
         ↓
   [동기화] 버튼 클릭 시
         ↓
   Supabase 업로드
...
|------|-----|-------------|
| 기본 동작 | 자동 동기화 | 로컬 저장만 |
| 동기화 | 백그라운드 (debounce) | 수동 버튼 |
| 오프라인 | 제한적 | 완전 지원 |
| 로그인 | 필수 (게스트 제외) | 선택 (동기화 시만) |

### 구현 항목 (TODO)

- [ ] 동기화 버튼 UI (헤더)
- [ ] 동기화 상태 표시 (마지막 동기화 시간)
- [ ] 상태: `pending`, `syncing`, `synced`, `conflict`, `offline`
- [ ] 충돌 해결 다이얼로그 ("로컬 유지" / "클라우드 유지")
- [ ] 오프라인 감지 및 버튼 비활성화
- [ ] Electron 환경 감지하여 자동 동기화 비활성화
