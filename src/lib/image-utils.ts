...
/**
 * 캐릭터 이미지를 150x150 정사각형으로 리사이즈
 * 중앙 크롭 후 리사이즈하여 비율 유지
 */
export async function resizeCharacterImage(file: File): Promise<Blob> {
  const img = await loadImage(file);

  const canvas = document.createElement("canvas");
  canvas.width = CHARACTER_SIZE;
  canvas.height = CHARACTER_SIZE;
  ...
  // 중앙 크롭 + 리사이즈
  const scale = Math.max(
    CHARACTER_SIZE / img.width,
    CHARACTER_SIZE / img.height
  );
  const scaledWidth = img.width * scale;
  const scaledHeight = img.height * scale;
  const x = (CHARACTER_SIZE - scaledWidth) / 2;
  const y = (CHARACTER_SIZE - scaledHeight) / 2;
...
/**
 * 캐릭터 이미지 파일을 리사이즈하고 Base64로 변환
 */
export async function processCharacterImage(file: File): Promise<string> {
  const resized = await resizeCharacterImage(file);
  return blobToBase64(resized);
}
